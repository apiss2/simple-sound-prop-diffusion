...
__C.TRAIN.DIFFUSION.B_MAP_MIN = (
    0.97  # or 1 if you want to use the original code, without b_maps
)
...
def add_base_args(parser, _cfg):
    parser.add_argument("--dataset_mode", default=_cfg.DATASETS.DATASET_MODE, type=str)
    parser.add_argument(
        "--learn_sigma",
        type=str2bool,
        nargs="?",
    ...
    parser.add_argument(
        "--noise_schedule", default=_cfg.TRAIN.DIFFUSION.NOISE_SCHEDULE, type=str
    )
    parser.add_argument(
        "--timestep_respacing",
    ...
    parser.add_argument("--img_size", default=_cfg.TRAIN.IMG_SIZE, type=int)
    parser.add_argument("--num_classes", default=_cfg.TRAIN.NUM_CLASSES, type=int)
    parser.add_argument("--lr", default=_cfg.TRAIN.LR, type=float)
    parser.add_argument(
        "--attention_resolutions", default=_cfg.TRAIN.ATTENTION_RESOLUTIONS, type=str
    )
    parser.add_argument("--channel_mult", default=_cfg.TRAIN.CHANNEL_MULT, type=int)
    parser.add_argument("--dropout", default=_cfg.TRAIN.DROPOUT, type=float)
    parser.add_argument(
        "--diffusion_steps", default=_cfg.TRAIN.DIFFUSION_STEPS, type=int
    )
    parser.add_argument(
        "--schedule_sampler", default=_cfg.TRAIN.SCHEDULE_SAMPLER, type=str
    )
    parser.add_argument("--num_channels", default=_cfg.TRAIN.NUM_CHANNELS, type=int)
    parser.add_argument("--num_heads", default=_cfg.TRAIN.NUM_HEADS, type=int)
    parser.add_argument(
        "--num_heads_upsample", default=_cfg.TRAIN.NUM_HEADS_UPSAMPLE, type=int
    )
    parser.add_argument(
        "--num_head_channels", default=_cfg.TRAIN.NUM_HEAD_CHANNELS, type=int
    )
    parser.add_argument("--num_res_blocks", default=_cfg.TRAIN.NUM_RES_BLOCKS, type=int)
    parser.add_argument(
        "--resblock_updown",
        type=str2bool,
        nargs="?",
    ...
    parser.add_argument("--weight_decay", default=_cfg.TRAIN.WEIGHT_DECAY, type=float)
    parser.add_argument(
        "--lr_anneal_steps", default=_cfg.TRAIN.LR_ANNEAL_STEPS, type=int
    )
    parser.add_argument("--batch_size_train", default=_cfg.TRAIN.BATCH_SIZE, type=int)
    parser.add_argument("--ema_rate", default=_cfg.TRAIN.EMA_RATE, type=str)
    parser.add_argument("--drop_rate", default=_cfg.TRAIN.DROP_RATE, type=float)
    parser.add_argument("--log_interval", default=_cfg.TRAIN.LOG_INTERVAL, type=int)
    parser.add_argument("--save_interval", default=_cfg.TRAIN.SAVE_INTERVAL, type=int)
    parser.add_argument("--resume_checkpoint", default=_cfg.TRAIN.RESUME_CHECKPOINT)
    parser.add_argument(
    ...
    parser.add_argument(
        "--fp16_scale_growth", default=cfg.TRAIN.FP16_SCALE_GROWTH, type=float
    )
    parser.add_argument("--num_workers", default=_cfg.TRAIN.NUM_WORKERS, type=int)
    parser.add_argument(
        "--no_instance",
        type=str2bool,
        nargs="?",
    ...
    parser.add_argument(
        "--is_train", type=str2bool, nargs="?", const=True, default=_cfg.TRAIN.IS_TRAIN
    )
    parser.add_argument("--s", default=_cfg.TEST.S, type=float)
    parser.add_argument(
        "--inference_on_train",
        type=str2bool,
        nargs="?",
    ...
    parser.add_argument("--batch_size_test", default=_cfg.TEST.BATCH_SIZE, type=int)
    parser.add_argument(
        "--clip_denoised",
        type=str2bool,
        nargs="?",
    ...
    parser.add_argument("--num_samples", default=_cfg.TEST.NUM_SAMPLES, type=int)
    parser.add_argument("--results_dir", default=_cfg.TEST.RESULTS_DIR, type=str)
    parser.add_argument("--output_dir", default=_cfg.TRAIN.CHECKPOINT_DIR, type=str)

    parser.add_argument(
        "--b_map_min", default=_cfg.TRAIN.DIFFUSION.B_MAP_MIN, type=float
    )
    parser.add_argument(
        "--b_map_scheduler_type", default=_cfg.TRAIN.B_MAP_SCHEDULER_TYPE, type=str
    )
    parser.add_argument(
        "--preserve_length", default=_cfg.TRAIN.DIFFUSION.PRESERVE_LENGTH, type=str2bool
    )
    parser.add_argument(
        "--add_buffer", default=_cfg.TRAIN.DIFFUSION.ADD_BUFFER, type=str2bool
    )
