...
class UNet2DSISModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.time_embedding = TimestepEmbedding(
            block_out_channels[0], self.time_emb_dim
        )
        # Encoder
    ...
    def forward(self, x, timesteps, cond=None) -> UNet2DOutput:
        ...
        if cond is None:
            ...
            if cond.shape == torch.Size([b, h, w]):
                ...
                cond = (
                    F.one_hot(cond.long(), num_classes=self.cls_count)
                    .to(self.device)
                    .float()
                    .permute(0, 3, 1, 2)
                )
        ...
        assert cond.shape == torch.Size([b, self.cls_count, h, w]), (
    ...
    def get_config(
        ...
        assert str(img_size) in configuration.keys(), (
