...
def pick_tensor(
    ...
    t: Union[torch.Tensor, np.ndarray, int],
):
    ...
    """
    assert isinstance(values, (list, torch.Tensor, np.ndarray)), (
        "values should be a list, a tensor or an array"
    )
    assert isinstance(t, (int, torch.Tensor, np.ndarray)), (
        "t should be an int, a tensor or an array"
    )
...
class DDPMTrainingScheduler(DDPMScheduler):
    ...
    def _get_q_mean_variance(
        self, timesteps: torch.Tensor, sample_0: torch.Tensor, sample_t: torch.Tensor
    ):
        ...
        coef_xt = (
            torch.sqrt(current_alpha_ts)
            * (1 - alpha_prod_ts_prev)
            / (1 - alpha_prod_ts)
        )
    ...
    def _get_variances(
        self, timesteps: torch.Tensor, predicted_variances: torch.Tensor = None
    ):
        ...
        variances = torch.clamp(
            variances, min=1e-5
        )  # Important for mixed precision not to have a too low value.
        ...
        if variance_type == "fixed_small":
            ...
            log_variances = frac * adapt_tensor(max_log, frac) + (
                1 - frac
            ) * adapt_tensor(min_log, frac)
            variances = torch.exp(log_variances)

        return variances, log_variances

    def _get_p_mean_variance(
        self, model_output: torch.Tensor, timesteps: torch.Tensor, scale: float = 1.0
    ):
        ...
        if scale > 1.0:
            ...
            model_var, model_log_var = self._get_variances(
                timesteps, model_var.chunk(2, dim=0)[0]
            )
