...
from losses import VLBLoss
from models import UNet2DSISModel
from pipeline import SemanticOnlyDDMPipeline
from schedulers import DDPMScheduler, DDPMTrainingScheduler
...
parser.add_argument(
    "--ddp_guidance_scale", type=float, default=7.5, help="Unconditionnal guidance."
)
parser.add_argument(
...
parser.add_argument(
    "--optim_max_grad_norm", default=1.0, type=float, help="Max gradient norm."
)
parser.add_argument(
    "--learning_rate",
    type=float,
...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="Number of training epochs before validation...",
)
...
def main(
    ...
    accelerator_project_configuration = ProjectConfiguration(
        project_dir=project_dir, logging_dir=logdir_path
    )
    ...
    if use_ema:
        ema_unet = EMAModel(
            unet.parameters(), model_cls=UNet2DSISModel, model_config=unet.config
        )
    ...
    def load_model_hook(models: List[UNet2DSISModel], input_dir):
        if use_ema:
            load_model = EMAModel.from_pretrained(
                os.path.join(input_dir, "unet_ema"), UNet2DSISModel
            )
    ...
    optimizer = optimizer_class(
        unet.parameters(), lr=learning_rate, weight_decay=optim_weight_decay
    )
    ...
    noise_scheduler = DDPMTrainingScheduler(
        variance_type=ddp_variance_type, beta_schedule=ddp_training_beta_type
    )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.autocast():
                ...
                cond = (
                    F.one_hot(y.long(), num_classes=dataset_cls_count)
                    .to(x.device)
                    .float()
                    .permute(0, 3, 1, 2)
                )
                ...
                ucond = (
                    torch.rand(len(y), device=x.device) < train_ucond_prob
                )  # 1 if < p_ucond
                cond = cond * (
                    1.0 - ucond[..., None, None, None].float()
                )  # if ucond, we multiply by 0.
                # We sample the noise
                ...
                if not learned_variance:
                    ...
                    model_pred, _, logvar_pred = noise_scheduler._get_p_mean_variance(
                        model_output, timesteps
                    )
                    # Get q variance with DDPM Scheduler
                    q_mean, _, q_logvar = noise_scheduler._get_q_mean_variance(
                        timesteps, x, x_t
                    )
                    ...
                    vlb_loss += (
                        accelerator.gather(
                            vlb_loss_batch.repeat(train_batch_size)
                        ).mean()
                        / gradient_accumulation_steps
                    )

                # We compute the Loss_simple value...
                loss_simple_batch = F.mse_loss(
                ...
                simple_loss += (
                    accelerator.gather(
                        loss_simple_batch.repeat(train_batch_size)
                    ).mean()
                    / gradient_accumulation_steps
                )
                total_loss += (
                    accelerator.gather(loss_total_batch.repeat(train_batch_size)).mean()
                    / gradient_accumulation_steps
                )
                    ...
                    if accelerator.is_main_process:
                        ...
                        if checkpointing_total_limit is not None:
                            ...
                            if len(checkpoints) >= checkpointing_total_limit:
                                num_to_remove = (
                                    len(checkpoints) - checkpointing_total_limit + 1
                                )
                                ...
                                for removing_checkpoint in removing_checkpoints:
                                    removing_checkpoint = os.path.join(
                                        project_dir, removing_checkpoint
                                    )
                                    shutil.rmtree(removing_checkpoint)

                        save_path = os.path.join(
                            project_dir, f"checkpoint-{global_step}"
                        )
        ...
        if accelerator.is_main_process:
            ...
            if epoch % val_every_nepochs == 0:
                ...
                # create pipeline
                pipeline = SemanticOnlyDDMPipeline(
                    unet=accelerator.unwrap_model(unet), scheduler=val_scheduler
                )
